...
class CombatHandler:
    ...
    def spell_power_coefficient(self, spell_id, effect_slot=4):
        ...
        def spell_downrank_penalty():
            spell_base_level = spell_from_db[DB.spell_column_info["BaseLevel"]]
            spell_max_level = spell_from_db[DB.spell_column_info["MaxLevel"]]

            downrank_multiplier = (spell_max_level + 6) / self.char.level
            if downrank_multiplier > 1:
                downrank_multiplier = 1

            if spell_base_level < 20:
                sub20downrank = 1 - (20 - spell_base_level) * 0.0375
                if sub20downrank < 0:
                    sub20downrank = 0
                downrank_multiplier *= sub20downrank

            return downrank_multiplier
    ...
    def process_direct_damage_spell(self, spell_info, effect_slot):
        self.on_spell_impact(spell_info[0])
        if self.spell_does_hit(spell_info[0]):
            ...
            self.on_spell_resist(spell_info[0])
            self.logg(spell_info[DB.spell_column_info["SpellName"]] + " " +
    ...
    def energize(self, spell_info, effect_slot, item_id=0):
        ...
        # Improved Mana Gems Aura from serpent coil braid
        if spell_info[DB.spell_column_info["SpellFamilyFlags"]] & 1099511627776:
            for aura in self.active_auras:
                if aura.spell_id == 37447 and aura.aura_id == 108 and aura.misc_value == 8:
                    mana_restored = round(mana_restored * (1 + aura.value * aura.curr_stacks / 100))
    ...
    def proc_aura_charge(self, aura):
        aura.procced()
        if aura.proc[3] - aura.proc_counter == 0:
    ...
    def on_spell_impact(self, spell_id):
        spell_info = DB.get_spell(spell_id)
        # Mage Arcane Blast
        if spell_info[DB.spell_column_info["SpellFamilyName"]] == 3 and \
                spell_info[DB.spell_column_info["SpellFamilyFlags"]] & 0x20000000:
            self.apply_spell_effect(36032)

        for aura in self.get_procable_auras(proc_flags=0x15410):
            if self.aura_applies_to_spell(aura, spell_id):
                if aura.proc[2] >= random.randint(0, 100):
                    self.handle_aura_proc(aura, spell_id)

    def on_spell_resist(self, spell_id):
        for aura in self.get_procable_auras(proc_flags_ex=8):
            if self.aura_applies_to_spell(aura, spell_id):
                if aura.proc[2] >= random.randint(0, 100):
                    self.handle_aura_proc(aura, spell_id)

    def on_spell_hit(self, spell_id):
        for aura in self.get_procable_auras(proc_flags_ex=1):
            if self.aura_applies_to_spell(aura, spell_id):
                if aura.proc[2] >= random.randint(0, 100):
                    self.handle_aura_proc(aura, spell_id)

    def on_spell_crit(self, spell_id, damage=0):
        for aura in self.get_procable_auras(proc_flags_ex=2):
            if self.aura_applies_to_spell(aura, spell_id):
                if aura.proc[2] >= random.randint(0, 100):
                    self.handle_aura_proc(aura, spell_id)

                if aura.spell_id in [11119, 11120, 12846, 12847, 12848] and DB.get_spell_school(spell_id) & 4:
        ...
        for aura in [aura for aura in self.active_auras if aura.spell_id == 11129]:
    ...
    def handle_aura_proc(self, aura, spell_id):
        # proc all trigger spells that are not winters chill
    ...
    def item_start_cooldown(self, item_info):
        for i in range(1, 6):
            if item_info[DB.item_column_info["spellid_" + str(i)]]:
                if item_info[DB.item_column_info["spellcooldown_" + str(i)]] > 0:
                    self.cooldown_item_id[item_info[0]] = self.env.now \
                                                          + item_info[DB.item_column_info["spellcooldown_" + str(i)]]
                if item_info[DB.item_column_info["spellcategorycooldown_" + str(i)]] > 0:
                    spell_category = item_info[DB.item_column_info["spellcategory_" + str(i)]]
                    spell_category_cooldown = item_info[DB.item_column_info["spellcategorycooldown_" + str(i)]]
    ...
    def use_item(self, item_id):
        ...
        for i in range(1, 6):
            ...
            if item_spell_id and item_info[DB.item_column_info["spelltrigger_" + str(i)]] == 0:
                self.apply_spell_effect(item_spell_id, item_id)
                self.item_start_cooldown(item_info)

                self.char.active_consumables[item_id] += 1
                # remove some active items after charges used ie. mana gems
        ...
        # if item is mana gem and improved mana gems spell(from serpent coil braid) is active
        if item_id in (5513, 5514, 8007, 8008, 22044) and any(aura.spell_id == 37447 for aura in self.active_auras):
            # trigger mana surge effect on mana gem use
            self.apply_spell_effect(37445)
